...
2. Put a single file at `webhose/webz_news_search/webz_news_search-0.1.0.difypkg`
3. Open a PR to `main` in English, one package per PR
4. `author` in manifest.yaml must match the GitHub handle that opens the PR
...
If the PR is opened from `ori-webz` instead of the `webhose` org, change
`author` in `manifest.yaml`, `provider/webz.yaml`, and the tool YAML to that
handle, then repackage.
